...
int main(int argc, char *argv[]) {
    ...
    if (mode == MODE_READ) {
        ...
        //elapsedTime += (t2.tv_usec - t1.tv_usec) / 1000.0;
         read_speed = (double) (sizeof(value) * 8 * count * epoch) / (double) elapsedTime;
        printf("%f us.\n", elapsedTime);
        printf("Read Speed = %lf Mbps\n", read_speed);
        /*for (int i = 0; i < count; i++) {
           printf("0x%08x\n", arr[i]); 
        }*/
    } 
        ...
        elapsedTime = ((t2.tv_sec - t1.tv_sec) * 1000000L + (t2.tv_usec - t1.tv_usec));
        write_speed = (double) (sizeof(value) * 8 * count * epoch) / (double) elapsedTime;
        //elapsedTime += (t2.tv_usec - t1.tv_usec) / 1000.0;
        printf("%f us.\n", elapsedTime); 
        printf("Write Speed = %lf Mbps\n", write_speed);
    }
